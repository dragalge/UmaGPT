...
TRACKBLAZER_SHOP_CHECKMARK_PATH = "assets/trackblazer/checkmark.png"
...
TRACKBLAZER_MAX_REASONABLE_HELD_QTY = TRACKBLAZER_INVENTORY_QTY_MAX
...
TRACKBLAZER_SHOP_INITIAL_LOAD_WAIT = 5.0
...
TRACKBLAZER_INVENTORY_SCROLL_SETTLE_WAIT = 0.25
...
TRACKBLAZER_SHOP_ROW_MIN_VISIBLE_TOP_MARGIN = 2
TRACKBLAZER_SHOP_ROW_MIN_VISIBLE_BOTTOM_MARGIN = 1
...
def _read_trackblazer_qty_digit_templates_from_disk():
  ...
  if templates:
    info(f"[TRACKBLAZER] Loaded quantity digit templates: {sorted(templates.keys())}")
  return templates
...
def load_trackblazer_item_catalog(force_reload=False):
  # Backward-compatible alias kept for existing callsites.
  return preload_trackblazer_item_catalog(force_reload=force_reload)
...
def _extract_price_from_match_region(screenshot, match, fallback_cost):
  ...
  price_crop = screenshot[top:bottom, left:right]
...
def _extract_shop_coin_count(threshold=0.85):
  ...
  detected = None
  if variant_votes:
    vote_counts = Counter(variant_votes)
    detected = sorted(vote_counts.keys(), key=lambda value: (-vote_counts[value], value))[0]
  elif candidates:
...
def _extract_held_quantity_from_match_region(screenshot, match, default_quantity=0, item_name="unknown_item"):
  x, y, w, h = match
  image_h, image_w = screenshot.shape[:2]
  safe_item_name = re.sub(r"[^a-zA-Z0-9_\-]", "_", str(item_name))

  # Crop to the right side of the icon where "Held <qty>" appears on the same row.
  left = min(max(x + w + 8, 0), image_w)
  right = min(max(x + w + 250, 0), image_w)
  top = min(max(y - 4, 0), image_h)
  bottom = min(max(y + h + 14, 0), image_h)

  if right <= left or bottom <= top:
    return default_quantity

  qty_crop = screenshot[top:bottom, left:right]
  if qty_crop.size == 0:
    return default_quantity

  if args.debug is not None:
    debug_window(qty_crop, save_name=f"trackblazer_qty_row_{safe_item_name}")
    info(
      f"[TRACKBLAZER] Qty OCR row bbox for '{item_name}': "
      f"(x1={left}, y1={top}, x2={right}, y2={bottom})"
    )

  qty_text = extract_text(Image.fromarray(qty_crop))
  enhanced_qty_crop = enhance_image_for_ocr(qty_crop, resize_factor=3)
  enhanced_qty_text = extract_text(enhanced_qty_crop)
  row_candidates = []
  for qty_source_text in (qty_text, enhanced_qty_text):
    if not qty_source_text:
      continue

    # Only trust an immediate single-digit quantity right after Held to avoid effect-stat digits like 43/47.
    held_value_match = re.search(r"(?i)held[^0-9]{0,4}([1-5])", qty_source_text)
    if held_value_match:
      try:
        row_candidates.append(int(held_value_match.group(1)))
      except ValueError:
        pass

  if row_candidates:
    row_counts = Counter(row_candidates)
    return sorted(row_counts.keys(), key=lambda value: (row_counts[value], value), reverse=True)[0]

  # Fallback numeric slots near the held quantity position.
  candidates = []
  numeric_slots = (
    (x + w + 146, x + w + 222, y - 1, y + h + 8),
    (x + w + 126, x + w + 214, y - 2, y + h + 10),
  )
  for n_left, n_right, n_top, n_bottom in numeric_slots:
    narrow_left = min(max(n_left, 0), image_w)
    narrow_right = min(max(n_right, 0), image_w)
    narrow_top = min(max(n_top, 0), image_h)
    narrow_bottom = min(max(n_bottom, 0), image_h)
    if narrow_right <= narrow_left or narrow_bottom <= narrow_top:
      continue
    ...
    if args.debug is not None:
      debug_window(
        narrow_crop,
        save_name=f"trackblazer_qty_slot_{safe_item_name}_{narrow_left}_{narrow_top}",
      )
      info(
        f"[TRACKBLAZER] Qty OCR slot bbox for '{item_name}': "
        f"(x1={narrow_left}, y1={narrow_top}, x2={narrow_right}, y2={narrow_bottom})"
      )
    qty_num = extract_number(narrow_crop)
    # Numeric-only fallback is noisy; only trust small values here.
    if 0 < qty_num <= TRACKBLAZER_INVENTORY_QTY_RESOLVED_MAX:
      candidates.append(int(qty_num))

    slot_text = extract_text(Image.fromarray(narrow_crop), allowlist="0123456789")
    enhanced_slot_crop = enhance_image_for_ocr(narrow_crop, resize_factor=3)
    enhanced_slot_text = extract_text(enhanced_slot_crop, allowlist="0123456789")
    for slot_source_text in (slot_text, enhanced_slot_text):
      for group in re.findall(r"\d+", slot_source_text):
        try:
          value = int(group)
          if TRACKBLAZER_INVENTORY_QTY_MIN <= value <= TRACKBLAZER_INVENTORY_QTY_RESOLVED_MAX:
            candidates.append(value)
        except ValueError:
          pass

  # Dedicated slot around the numeric value after "Held"; robust for bottom rows.
  held_value_left = min(max(x + w + 110, 0), image_w)
  held_value_right = min(max(x + w + 178, 0), image_w)
  held_value_top = min(max(y - 2, 0), image_h)
  held_value_bottom = min(max(y + h + 12, 0), image_h)
  if held_value_right > held_value_left and held_value_bottom > held_value_top:
    held_value_crop = screenshot[held_value_top:held_value_bottom, held_value_left:held_value_right]
    if args.debug is not None:
      debug_window(held_value_crop, save_name=f"trackblazer_qty_held_value_{safe_item_name}")
      info(
        f"[TRACKBLAZER] Qty OCR held-value bbox for '{item_name}': "
        f"(x1={held_value_left}, y1={held_value_top}, x2={held_value_right}, y2={held_value_bottom})"
      )
    held_value_num = extract_number(held_value_crop)
    if TRACKBLAZER_INVENTORY_QTY_MIN <= held_value_num <= TRACKBLAZER_INVENTORY_QTY_RESOLVED_MAX:
      candidates.append(int(held_value_num))

    template_qty, template_score = _match_quantity_from_digit_templates(held_value_crop, item_name=item_name)
    if template_qty is not None:
      if template_score >= TRACKBLAZER_QTY_DIGIT_TEMPLATE_HIGH_CONFIDENCE:
        # Template is calibrated to game font; high-confidence hit should win immediately.
        return int(template_qty)
      # Give moderate-confidence template matches stronger weight than plain OCR text.
      candidates.extend([int(template_qty), int(template_qty), int(template_qty)])

    held_value_text = extract_text(Image.fromarray(held_value_crop), allowlist="0123456789")
    enhanced_held_value_crop = enhance_image_for_ocr(held_value_crop, resize_factor=3)
    enhanced_held_value_text = extract_text(enhanced_held_value_crop, allowlist="0123456789")
    for value_text in (held_value_text, enhanced_held_value_text):
      for group in re.findall(r"\d+", value_text):
        try:
          value = int(group)
          if TRACKBLAZER_INVENTORY_QTY_MIN <= value <= TRACKBLAZER_INVENTORY_QTY_RESOLVED_MAX:
            candidates.append(value)
  ...
  if candidates:
    counts = Counter(candidates)
    return sorted(counts.keys(), key=lambda value: (counts[value], value), reverse=True)[0]
...
def _log_shop_timing(step, started_at, extra=""):
  ...
  info(f"[TRACKBLAZER][TIMING] {step}: {elapsed:.3f}s{suffix}")
...
def _close_trackblazer_overlay(threshold=0.85, context_label="overlay"):
  ...
  if close_btn_center:
    info(f"[TRACKBLAZER] close_btn center coordinates ({context_label}): {close_btn_center}")
...
def _back_out_of_trackblazer_shop(threshold=0.85):
  ...
  if back_btn_center:
    info(f"[TRACKBLAZER] back_btn center coordinates (shop): {back_btn_center}")
...
def _find_shop_row_anchors(shop_screenshot):
  ...
  template_specs = (
    ...
    (TRACKBLAZER_SHOP_CHECKMARK_PATH, "selected", 2),
...
def _shop_screen_contains_purchased_row(screenshot):
  rows = _find_shop_row_anchors(screenshot)
  return any(row.get("state") == "purchased" for row in rows)
...
def collect_trackblazer_shop_snapshot(item_catalog, threshold=0.85, max_scrolls=20):
  total_started_at = perf_counter()
  info("[TRACKBLAZER][TIMING] shop flow started")
  ...
  def _process_row(row, screenshot, panel_h, panel_w, dbg, label=""):
    ...
    if row_state == "purchased":
      info("[TRACKBLAZER] Encountered purchased row anchor — stopping discovery.")
    ...
    info(f"[TRACKBLAZER] {label}Shop row '{name}' at ({cx},{cy}) price={entry['price']} state={row_state}")
  ...
  buy_candidates.sort(key=lambda item: (-item.get("priority", 0), item.get("price", 0), item.get("name", "")))
  ...
  if shop_coins is not None and shop_coins > 0:
    ...
    while True:
      ...
      pending_selection = any(item.get("name", "") not in selected_set for item in planned_checkout)
...
def collect_trackblazer_owned_items_snapshot(item_catalog, threshold=0.85, max_scrolls=20):
  ...
  inventory_screen = device_action.screenshot(region_ltrb=TRACKBLAZER_INVENTORY_PANEL_LTRB)
  ...
  for _ in range(max_scrolls):
    ...
    for center_y, name, center_x, quantity_read, quantity_chosen in sorted(pass_matches, key=lambda row: row[0]):
      if quantity_read != quantity_chosen:
        ...
        info(f"[TRACKBLAZER] Inventory match '{name}' at ({center_x}, {center_y}), held={quantity_chosen}")
  ...
  if owned_items:
    ...
    info(f"[TRACKBLAZER] Inventory quantities detected: {inventory_summary}")
  else:
    info("[TRACKBLAZER] Inventory quantities detected: {}")
  return owned_items


def collect_trackblazer_item_snapshot(item_catalog):
  owned_items = collect_trackblazer_owned_items_snapshot(item_catalog)
  shop_items, shop_coins, purchased_items = collect_trackblazer_shop_snapshot(item_catalog)
...
class TrackblazerHandler(ScenarioHandler):
  ...
  def _inventory_diff_lines(self, expected_counts, actual_counts):
    lines = []
    all_names = sorted(set(expected_counts.keys()) | set(actual_counts.keys()))
    for name in all_names:
      expected_qty = expected_counts.get(name, 0)
      actual_qty = actual_counts.get(name, 0)
      if expected_qty != actual_qty:
        lines.append(f"{name}: expected {expected_qty}, actual {actual_qty}")
    return lines
  ...
  def _sync_trackblazer_items_for_turn(self, turn_token):
    item_catalog = preload_trackblazer_item_catalog()

    # Start-of-turn validation: actual inventory must match expected state from previous turn purchases.
    owned_items = collect_trackblazer_owned_items_snapshot(item_catalog)
    actual_counts = self._inventory_counts_from_owned_items(owned_items)
    info(
      f"[TRACKBLAZER] Turn {turn_token} inventory quantities: "
      f"{self._format_inventory_counts_for_log(actual_counts)}"
    )
    if not self.expected_inventory_counts:
      info("[TRACKBLAZER] Expected inventory initialized from current inventory snapshot.")
    else:
      diffs = self._inventory_diff_lines(self.expected_inventory_counts, actual_counts)
      if diffs:
        warning(f"[TRACKBLAZER] Inventory mismatch at turn {turn_token}: {'; '.join(diffs)}")
      else:
        info(f"[TRACKBLAZER] Inventory validation passed at turn {turn_token}.")

    # Resync expected baseline to actual, then add this turn's purchases after shop check.
    self.expected_inventory_counts = dict(actual_counts)

    # Check shop every turn and optionally purchase.
    shop_items, shop_coins, purchased_items = collect_trackblazer_shop_snapshot(item_catalog)
  ...
  def collect_main_state_patch(self, state: dict[str, object]) -> dict[str, object]:
    ...
    if turn_token == self.last_turn_synced:
      return {}

    info(f"[TRACKBLAZER] Start-of-turn item sync for timeline token: {turn_token}")
    self._sync_trackblazer_items_for_turn(turn_token)
    self.last_turn_synced = turn_token
    return {}

  def on_turn_read(self, turn_text: str) -> None:
    # Intentionally no-op: Trackblazer sync now keys off state["year"] in collect_main_state_patch.
    return
